...
import { getEmployees, createEmployee, getEmployee, updateEmployee, generateEmployeeId } from "../api/employees.ts";
...
export function AddEmployee() {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      // Generate Employee ID from backend if it's a new employee
      if (!id) {
        try {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Append payload fields directly to FormData
    for (const key in payload) {
      if (payload[key] !== undefined && payload[key] !== null) {
        if (typeof payload[key] === 'object') {
          employeeFormData.append(key, JSON.stringify(payload[key]));
        } else {
          employeeFormData.append(key, String(payload[key]));
        }
      }
    }
    ...
    if (profilePhoto) {
      employeeFormData.append('profile_picture', profilePhoto);
    }
    if (cvFile) {
    ...
    try {
      if (id) {
        await updateEmployee(parseInt(id, 10), employeeFormData);
